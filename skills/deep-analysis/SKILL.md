...
2. ⚠️ **每个 HIGH 风险/死因 → 给应对方案,必须带等级**:🟢 实证(用 `search_github` / `read_github_file` 查同类成熟开源项目,带 repo@文件:行号)/ 🟡 推理(未查,注明原因)。HIGH(后果=钱/时间/承诺)必须 🟢,只给 🟡 = 不合格,立即补查;低危允许 🟡。这是"检查到风险时重新查 GitHub 给出完善方案"——不要拿着第一版方案直接下裁决。⚠️ **查询预算与 HIGH 数量挂钩**:整个整合环节 ≤ min(HIGH 数, 12) 次,每个 HIGH 最多 1 次,**优先保证 HIGH 的 🟢**;⚠️ **同类风险合并查询**:同一领域/很可能同一仓库的风险(如 websocket 路由 + 坐席分配可能指向同一客服项目)一次读文件查证多个;读文件遵守读取纪律(>300 行用 curl+sed 分段,整文件 read_github_file ≤2 个);⚠️ **预算耗尽出口**:预算用完后的剩余 HIGH 不再新查询,给推理方案标 🟡(注明预算耗尽),并在闸门 3 报告列为已知局限(不得装作已查证)。

#* 方案规则(必须给方案,两级标注): 上面每条 HIGH 死因的「应对方案」必须给出 1 主案 + 1-2 备选,不留空。方案标级:🟢 实证 = 查过 GitHub,带 repo@文件:行号;🟡 推理 = 未查,基于经验(注明原因)。HIGH 后果=真金白银/开发时间/公开承诺 → 必须 🟢,只给 🟡 = 不合格,立即补查。判据:该死因后果要投入(钱/时间/承诺)吗?是→必查;否→可推理。

3. **建议裁决**: proceed / 改(proceed-with-changes) / 转向(pivot) / kill。

展示给用户(一次合并展示,不额外加确认闸门):
- 调研摘要(三层命中数 / Sufficiency Gate 状态 / 复用候选前 3 个)
- ⭐ 遗漏清单(simulator 主动发现,重点;每条含解决方案,标 🟢/🟡)
- 已知风险(基于 GitHub 代码)
- Claim 列表前 5 条
- ✅ 可行性+风险小结(死因自检命中 → 每条给应对方案表:1 主案 + 1-2 备选,标 🟢/🟡;+ 建议裁决)
...
2. ⚠️ **每条 🔴/🟨 攻击 → 解决方案,必须带等级**:🔴(杀伤力最高)必须 🟢 实证(用 `search_github` / `read_github_file` 查同类成熟开源项目怎么应对,带 repo@文件:行号),只给 🟡 = 不合格,立即补查;🟨 允许 🟡 推理(注明原因)。⚠️ **工具上限**:整个对抗环节 GitHub 查询 ≤ min(🔴/🟨 数, 15) 次,每条 🔴/🟨 攻击最多 1 次;⚠️ **同类攻击合并查询**:多条攻击指向同一仓库/同一问题时一次读文件查证多个;⚠️ **预算耗尽出口**:超上限 → 剩余攻击给推理方案标 🟡(注明预算耗尽),并在闸门 3 报告列为已知局限(不得装作已查证)。防止对抗环节 token 膨胀。
...
| 闸门 2 | researcher ≤15 次工具调用(⚠️ 读文件前先探测大小,>300 行禁用 read_github_file 整读、改用 curl+sed 分段,整文件读 ≤3 次,每仓库 ≤3 文件,报告 800-1200 字),simulator ≤10 次(读码 ≤4 次含 curl+sed,输出 500-800 字),超上限强制推进;不写文件,markdown 直接返回;⚠️ 每个 HIGH 风险给应对方案且必须 🟢实证(编排层整合 ≤ min(HIGH 数,12) 次,同类风险合并查询,优先保证 HIGH 的 🟢,预算耗尽给推理方案标 🟡 并列为报告局限);必须显式裁决;⚠️ **深入续跑第二轮预算减半**(researcher ≤8 / simulator ≤5,只出增量) |
| 闸门 2.5 | ⚠️ 对抗决策**合并进闸门 2 收尾确认**(与方向确认一次问完);即使用户说「直接下一步」/「确认」也必须追问对抗问题,答完才进闸门 3,不得省略 |
| 对抗(可选) | 红队攻击必须具体到数字/机制,不得泛泛;每条攻击必须有响应,🔴 必须 🟢实证(查 GitHub 给方案,带 repo@文件:行号),🟨 可 🟡推理;整个对抗 GitHub 查询 ≤ min(🔴/🟨 数,15) 次,同类攻击合并查询,预算耗尽给推理方案标 🟡 并列为报告局限,同样遵守读取纪律;跳过则无攻击面 |
