...
#* 查证门(先查后标): 上面每条 HIGH 的「应对方案」必须先发起真实查询(curl / gh / MCP / web_search 任一,引用落到源码文件才算实证);只有「查询后确认无同类开源实现」或「查询预算已耗尽」才准标「未查码=推测」,且必须附原因(试过什么通道+关键词,或预算耗尽)。没查就标推测 = 违规,立即补查。两者皆非 → 不合格回查,不得带过。
